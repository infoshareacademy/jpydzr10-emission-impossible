...
class ConvertViaFuelTests(TestCase):
    ...
    def test_volume_liters_falls_back_to_convert(self):
        ...
        # Pint potrafi konwertować l <-> kg tylko dla wody (domyślnie),
        # więc testujemy czy nie rzuci błędu dla obsługiwanych przypadków
    ...
    def test_unsupported_path_falls_back(self):
        ...
        # To rzuci DimensionalityError, bo pint nie potrafi MJ -> kg
        with self.assertRaises(DimensionalityError):
...
class CalculateEmissionTests(TestCase):
    """Testy końcowego obliczania emisji CO2e."""
    ...
    def test_simple_energy_factor(self):
        """Podstawowy przypadek: ilość w MWh, wskaźnik w tCO2e/MWh."""
        result = calculate_emission(
            amount=500,
            unit="MWh",
            factor_value=0.82,
            unit_factor="tCO2e/MWh",
            fuel_spec=None,
        )
        self.assertAlmostEqual(result, 410.0)  # 500 * 0.82

    def test_energy_with_conversion(self):
        """kWh -> MWh, potem przez wskaźnik."""
        result = calculate_emission(
            amount=1000,
            unit="kWh",
            factor_value=0.5,
        ...
        self.assertAlmostEqual(result, 0.5)  # 1 MWh * 0.5

    def test_volume_to_energy_via_fuel(self):
        """m3 paliwa -> MJ -> przez wskaźnik."""
        result = calculate_emission(
            amount=2,
            unit="m3",
            factor_value=0.0001,  # tCO2e/MJ
            unit_factor="tCO2e/MJ",
            fuel_spec=self.fuel,
        )
        # 2 m3 * 36300 MJ/m3 = 72600 MJ; * 0.0001 = 7.26 tCO2e
        self.assertAlmostEqual(result, 7.26)

    def test_mass_to_energy_via_fuel(self):
        """kg paliwa -> MJ -> przez wskaźnik."""
        result = calculate_emission(
            amount=100,
            unit="kg",
            factor_value=0.0001,
        ...
        # 100 kg * 42.7 MJ/kg = 4270 MJ; * 0.0001 = 0.427 tCO2e
        self.assertAlmostEqual(result, 0.427)

    def test_volume_to_mass_factor(self):
        """m3 -> kg przez gęstość, wskaźnik w tCO2e/kg."""
        result = calculate_emission(
            amount=10,
            unit="m3",
            factor_value=0.003,
            unit_factor="tCO2e/kg",
            fuel_spec=self.fuel,
        )
        # 10 m3 * 850 kg/m3 = 8500 kg; * 0.003 = 25.5 tCO2e
        self.assertAlmostEqual(result, 25.5)

    def test_factor_without_denominator(self):
        """Wskaźnik bez mianownika – brak konwersji ilości."""
        result = calculate_emission(
            amount=100,
            unit="szt",  # jednostka nie ma znaczenia, bo nie ma konwersji
            factor_value=0.5,
            unit_factor="tCO2e",
            fuel_spec=None,
        )
        self.assertAlmostEqual(result, 50.0)  # 100 * 0.5

    def test_result_in_kgco2e_converted_to_tco2e(self):
        """Wynik w kgCO2e automatycznie konwertowany do tCO2e."""
        result = calculate_emission(
            amount=1,
            unit="MWh",
            factor_value=500,
            unit_factor="kgCO2e/MWh",
            fuel_spec=None,
        )
        self.assertAlmostEqual(result, 0.5)  # 500 kgCO2e = 0.5 tCO2e

    def test_result_already_in_tco2e(self):
        """Wynik w tCO2e bez dodatkowej konwersji."""
        result = calculate_emission(
            amount=100,
            unit="MWh",
            factor_value=0.25,
    ...
    def test_fuel_spec_none_with_compatible_units(self):
        """Bez fuel_spec, ale jednostki są kompatybilne przez pint."""
        result = calculate_emission(
            amount=5,
            unit="GJ",
            factor_value=0.1,
        ...
        # 5 GJ = 5000 MJ; 1 MWh = 3600 MJ; 5000/3600 ≈ 1.3889 MWh
        expected = (5000 / 3600) * 0.1
        self.assertAlmostEqual(result, expected, places=5)

    def test_missing_fuel_spec_raises_error(self):
        """Brak fuel_spec gdy jest potrzebny (np. m3 -> kg)."""
        # pint nie potrafi m3 -> kg bez gęstości, więc powinien być błąd
